(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global['remote-import'] = {}));
  ...
  // 远程代码载入执行
  function ImportScript(scripts, callback) {
      // 创建执行环境
      return Promise.all(scripts.map(function (item) {
          if (isString(item)) {
              return Promise.resolve(Parser(item));
          }
          if (isPromise(item)) {
              return item.then(function (data) { return Parser(data); });
          }
          if (isAst(item)) {
              return item;
          }
