...
export class Interpreter {
	...
	protected currentScope: Scope = new Scope({}, null, RootScopeName);
	protected globalScope: Scope = new Scope({}, null, GlobalScopeName);
	...
	protected execEndTime: number = 0;
	...
	protected arrayExpressionHandler(node: ESTree.ArrayExpression) {
		//fix: [,,,1,2]
		const items: Array<BaseClosure | null> = node.elements.map(element =>
	...
	protected catchClauseHandler(node: ESTree.CatchClause): (e: Error) => any {
		const paramNameGetter = this.createParamNameGetter(node.param as ESTree.Pattern);
		const bodyClosure = this.createClosure(node.body);
	...
	protected createMemberKeyGetter(node: ESTree.MemberExpression): Getter {
		...
		return node.computed
			...
			: this.createObjectKeyGetter(node.property as ESTree.Expression);
	}
